...
#   ./install.sh render opencode2         print the resolved OpenCode 2 config
#
...
ALL_MODULES="tmux nvim nvim-ai opencode opencode2 agents-skills claude-skills"
...
module_desc() {
	case "$1" in
		...
		opencode) echo "OpenCode 1 config (base${OPENCODE_OVERLAY:+ + $OPENCODE_OVERLAY overlay})" ;;
		opencode2) echo "OpenCode 2 config (base${OPENCODE_OVERLAY:+ + $OPENCODE_OVERLAY overlay})" ;;
...
# Print a resolved OpenCode config: base.jsonc as-is when no overlay,
...
render_opencode_version() {
	local version="$1" base="$REPO_DIR/.config/$1/base.jsonc"
	if [ -z "$OPENCODE_OVERLAY" ]; then
		cat "$base"
		return 0
	fi
	local overlay="$REPO_DIR/.config/$version/$OPENCODE_OVERLAY.jsonc"
	...
	if [ "$version" = opencode2 ]; then
		# V2 appends permission rules across config layers rather than replacing them.
		jq -s '
			.[0] as $base | .[1] as $overlay |
			($base * $overlay) |
			.permissions = (($base.permissions // []) + ($overlay.permissions // [])) |
			reduce ((($base.agents // {}) + ($overlay.agents // {})) | keys[]) as $agent (.;
				if (($base.agents[$agent].permissions // null) and ($overlay.agents[$agent].permissions // null))
				then .agents[$agent].permissions = ($base.agents[$agent].permissions + $overlay.agents[$agent].permissions)
				else . end
			)
...
render_opencode() { render_opencode_version opencode; }
render_opencode2() { render_opencode_version opencode2; }
...
install_opencode_version() {
	...
	if [ -z "$OPENCODE_OVERLAY" ]; then
		install_file ".config/$version/base.jsonc" "$dest"
	else
		local tmp last="$STATE_DIR/rendered-$version.jsonc"
		tmp="$(mktemp)"
		"render_$version" >"$tmp"
...
install_opencode() { install_opencode_version opencode; }

install_opencode2() {
	install_opencode_version opencode2
	install_file ".config/opencode2/cli.json" "$HOME/.config/opencode/cli.json"
}
...
apply_profile() {
	case "$1" in
		...
		personal)
			ENABLED="$(profile_modules personal)"
			OPENCODE_OVERLAY=""
			NVIM_PY_FMT=yes
			;;
		work)
			ENABLED="$(profile_modules work)"
...
toggle() {
	if enabled "$1"; then
		...
		case "$1" in
			opencode) enabled opencode2 && ENABLED="$(printf "%s" " $ENABLED " | sed 's/ opencode2 / /')" ;;
			opencode2) enabled opencode && ENABLED="$(printf "%s" " $ENABLED " | sed 's/ opencode / /')" ;;
		esac
		ENABLED="$ENABLED $1"
	fi
}
...
resolve_deps() {
	if enabled opencode && enabled opencode2; then
		die "opencode and opencode2 are mutually exclusive (both install to ~/.config/opencode/opencode.jsonc)"
	fi
...
run_menu() {
	...
	while true; do
		...
		case "$choice" in
			"") return 0 ;;
			q) exit 0 ;;
			a) ENABLED="$(profile_modules "$PROFILE")" ;;
			n) ENABLED="" ;;
	...
	case "$cmd" in
		render)
			case "$render_target" in
				opencode | opencode2) "render_$render_target" ;;
				*) die "render target must be 'opencode' or 'opencode2'" ;;
			esac
			;;
		list)
			list_modules
